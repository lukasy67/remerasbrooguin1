...
export default function App() {
  ...
  const [adminCodeUsed, setAdminCodeUsed] = useState("");
  ...
  const isHiddenAdmin = adminCodeUsed === "lukasy67";
...
const handleSaveVisualPrices = async () => {
  try {
    ...
    const existing = await supabaseRequest('global_settings?id=eq.global_pricing', 'GET');
    if (existing.error) throw new Error(existing.error);

    let saveResult;

    if (existing.data && existing.data.length > 0) {
      saveResult = await supabaseRequest(
        'global_settings?id=eq.global_pricing',
        'PATCH',
        { value: JSON.stringify(normalizedPrices) }
      );
    } else {
      saveResult = await supabaseRequest(
        'global_settings',
    ...
    // Verifica que realmente haya actualizado o creado una fila
    if (!saveResult.data || saveResult.data.length === 0) {
      throw new Error('Supabase no devolvió ninguna fila actualizada.');
    }
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {isAdmin && (
          <div className={`${t.card} border-l-4 border-indigo-500 p-4 rounded-r-xl shadow-md space-y-4`}>
            ...
            {isHiddenAdmin && (
  <button onClick={() => setShowSponsorsManager(true)}>
    Sponsors
  </button>
)}
